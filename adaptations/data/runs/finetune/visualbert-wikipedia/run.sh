...
HOME=$TMPDIR deepspeed --hostfile $TMPDIR/hostfile finetune.py \
    --text-dataset $DATA_DIR/wikipedia/finetune/{train,val}.jsonl \
    --evaluate-every 40 \
    --checkpoint-every 40 \
    --checkpoint-max 1 \
    --checkpoint-dir $CHECKPOINT_DIR/visualbert-wikipedia \
    --deepspeed_config ../data/runs/finetune/visualbert-wikipedia/deepspeed.config \
    --model "visualbert"
